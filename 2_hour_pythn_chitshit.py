...
print(str.isidentifier()) #False ,A string is considered a valid identifier if it only contains alphanumeric letters (a-z) and (0-9), or underscores (_). A valid identifier cannot start with a number, or contain any spaces.
...
c = {}.fromkeys(v,0) # {'a': 0, 'e': 0, 'i': 0, 'o': 0, 'u': 0} ,dictionary with each vowel a key and value 0
...
for a in string:
	if (a.isspace()) == True:
		count+=1
print(count) #8
#string lstrip()method:Remove spaces to the left of the string.have to start with char in left 
string = "geeks for geeks"
print(string.lstrip('ge')) #ks for geeks,it cut all g,e found leading left. if not started with g,it couldn't cut from left.
...
#Function as a Parameter of Another Function.You can pass functions around as parameters
def square_number (n):
    return n * n
...
regex_pattern = r'[a].+'  # . means any character, + means any character 1/more times including space&any word after space until another 'a' is found
matches = re.findall(regex_pattern, txt)
print(matches)  # ['and banana are fruits']
#Zero or many times. The pattern could may not occur or it can occur many times.
regex_pattern = r'[a].*'  # . any character, * any character 0/more times including space&any word after space until another 'a' is found
txt = '''Apple and banana are fruits'''
matches = re.findall(regex_pattern, txt)
print(matches)  # ['and banana are fruits']
